...
export function useTreasuryPositions({
  ...
  // ─── Pass 1: Read unallocated balance + share balances in all YO vaults ───
  const pass1Contracts = [
    // [0] Unallocated underlying
    ...(assetAddress
      ? [
          {
            chainId,
      ...
      : []),
    // [1..N] Share balances in YO vaults
    ...yoVaults.map((v) => ({
      chainId,
      address: v.address,
      abi: erc20Abi,
  ...
  const offset = assetAddress ? 1 : 0;
  const unallocatedBalance =
    assetAddress && pass1Data?.[0]?.status === 'success'
      ? (pass1Data[0].result as bigint)
      : undefined;

  const shareBalances = yoVaults.map((_, i) => {
    const result = pass1Data?.[i + offset];
